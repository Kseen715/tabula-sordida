...
- `.githooks/pre-commit` reruns both automatically when a commit touches `<LANG>/*.html` or `build_index.py`, and stages the new `index.html`. Enable it once per clone with `git config core.hooksPath .githooks`. It needs network, since `inline.py` downloads the index fonts.
